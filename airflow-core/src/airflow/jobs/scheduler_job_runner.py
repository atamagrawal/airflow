...
class SchedulerJobRunner(BaseJobRunner, LoggingMixin):
    ...
    def _create_shadow_dag_runs(
        self,
        production_dag_runs: list[DagRun],
        *,
        session: Session,
    ) -> None:
        """
        ...
        This implements the "Read parity" and "Write isolation" invariants from
        AIP-09 §4.  The shadow DagRun is a normal DagRun for the *candidate*
        DAG, distinguished by ``conf["__shadow_run__"]`` and
        ``conf["__shadow_id__"]``.
    ...
    def _cleanup_expired_shadows(self, *, session: Session) -> None:
        """
        Transition expired Shadow DAGs to CLEANED_UP status.

        Called on each scheduler heartbeat to ensure shadows that have passed
        their TTL are automatically archived without operator intervention.
        """
        try:
            from airflow.shadow.lifecycle import ShadowDagService

            cleaned = ShadowDagService().cleanup_expired(session=session)
            if cleaned:
